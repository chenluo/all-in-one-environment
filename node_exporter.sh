...
if [ $PRE = true ]; then
    ...
    NODE_EXPORTER_PACKAGE=node_exporter-1.4.0
    if [ -d $APP_DIR/$NODE_EXPORTER_PACKAGE ];then
        echo 'extracted'
    else
        if [ -f $APP_DIR/${NODE_EXPORTER_PACKAGE}.linux-amd64.tar.gz ];then
            echo 'downloaded'
            tar zxvf ${NODE_EXPORTER_PACKAGE}.linux-amd64.tar.gz
        else
            echo 'download and extracting'
            curl -o $APP_DIR/${NODE_EXPORTER_PACKAGE}.tar.gz -L $URL && tar zxvf ${NODE_EXPORTER_PACKAGE}.tar.gz
        fi
    fi
    ln -s $APP_DIR/node_exporter-1.4.0.linux-amd64 $WORKSPACE/node_exporter
